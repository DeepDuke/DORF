...
logger = MyLogger(cfg.log_path,  datetime.now().strftime("%m-%d-%Y_%H-%M-%S") + '_' + cfg.fined_log_filename, level=cfg.log_level) 
...
def save_ground_static_map(ground_point_raw_ids, pcd_raw_points, save_path):
    ...
    logger.INFO('There are {} / {} ground points detected'.format(len(ground_point_raw_ids), len(pcd_raw_points)))
...
def ground_segmentation_mp(pcd_raw_points, pcd_kdtree, node_msg_list, args):
    # result = mp.Pool(20).map(partial(ground_segmentation, pcd_raw_points=pcd_raw_points, pcd_kdtree=pcd_kdtree), node_msg_list)
    result = mp.Pool(20).map(partial(scan_ratio_test, pcd_raw_points=pcd_raw_points, pcd_kdtree=pcd_kdtree, args=args), node_msg_list)
...
def fined_main(pcd_raw_points, pcd_kdtree, node_msg_list,RESULT_SAVING_PATH, args):
    global logger
    
    ground_point_raw_ids, freq_dict, total_static_point_raw_ids, total_dynamic_point_raw_ids = ground_segmentation_mp(pcd_raw_points, pcd_kdtree, node_msg_list, args)
...
def pca_bin_ground_segmentation(bin, SOI_pts):
    ...
    if elevation_height < 0.2:
        z_threshold = 0.5*mean_z
    else:
        z_threshold = 0.1*elevation_height + min_z
    ...
    pca.fit(lower_bin_points)
    eigenvalues = pca.explained_variance_
    
    # Determine the direction of the ground plane
    min_eigenvalue_idx = np.argmin(eigenvalues)
    ground_normal = pca.components_[min_eigenvalue_idx]
    
    # Compute the signed distances of each point to the ground plane
    signed_distances = lower_bin_points.dot(ground_normal)
    # Identify the ground points by the signed distance threshold
    threshold = 0.2
    ground_mask = np.abs(signed_distances) < threshold
    result_ids = np.where(ground_mask == True)[0]
    
    ground_point_ids = [lower_bin_point_ids[id] for id in result_ids]
    
    return ground_point_ids 

def ransac_bin_ground_segmentaion(bin, SOI_pts, args, distance_threshold=0.1, max_iteration=1000):
    ...
    print('max_plane_z --> {} | min_plane_z --> {} | mean_plane_z --> {}'.format(max_plane_z, min_plane_z, min_plane_z))
...
def get_scan_VOI_info(node_msg, args):
    if args.dataset == 'kitti':
...
def get_scan_VOI_bin_list(scan_VOI_pts, args):
    if args.dataset == 'kitti':
        LOCAL_MAP_RADIUS = remo_params['LOCAL_MAP_RADIUS']
        N_RING = remo_params['n_ring']
        N_SEG = remo_params['n_seg']
    elif args.dataset == 'gazebo':
        LOCAL_MAP_RADIUS = remo_params['gazebo_local_map_radius']
        N_RING = remo_params['gazebo_n_ring']
        N_SEG = remo_params['gazebo_n_seg']
    elif args.dataset == 'ust':
        LOCAL_MAP_RADIUS = remo_params['ust_local_map_radius']
        N_RING = remo_params['ust_n_ring']
        N_SEG = remo_params['ust_n_seg']
...
def scan_ratio_test(node_msg, pcd_raw_points, pcd_kdtree, args):
    # Get scan bin list
    scan_VOI_pts = get_scan_VOI_info(node_msg, args)
    scan_bin_list = get_scan_VOI_bin_list(scan_VOI_pts, args)
    
    # Get submap bin list
    SOI_pts, SOI_pt_raw_ids = get_SOI_info(node_msg, pcd_raw_points, pcd_kdtree, args)
    submap_bin_list  = get_basic_bin_list(SOI_pts, args)
    
    # scan ratio checking
    result = map(partial(single_bin_scan_ratio_test, scan_VOI_pts=scan_VOI_pts, SOI_pts=SOI_pts, SOI_pt_raw_ids=SOI_pt_raw_ids, args=args), list(zip(scan_bin_list, submap_bin_list)))
...
def single_bin_scan_ratio_test(bin_pair, scan_VOI_pts, SOI_pts, SOI_pt_raw_ids, args):
    ...
    ground_point_ids, max_plane_z = ransac_bin_ground_segmentaion(submap_bin, SOI_pts, args)
...
def ground_segmentation(node_msg, pcd_raw_points, pcd_kdtree):
    # print('Starting ground_segmentation')
    # Get SOI
    SOI_pts, SOI_pt_raw_ids = get_SOI_info(node_msg, pcd_raw_points, pcd_kdtree)
    
    # Get bins
    bin_list = get_basic_bin_list(SOI_pts)
    
    # Perform bin-wise segmentations
    result_list = map(partial(ransac_bin_ground_segmentaion, SOI_pts=SOI_pts), bin_list)  
    # result_list = map(partial(pca_bin_ground_segmentation, SOI_pts=SOI_pts), bin_list)  
    
    ground_point_raw_ids = []
    for bin_result in result_list:
        ground_point_ids = bin_result[0]
        ground_point_raw_ids.extend([SOI_pt_raw_ids[id] for id in ground_point_ids])
    
    return ground_point_raw_ids

def get_basic_bin_list(SOI_pts, args):
    if args.dataset == 'kitti':
        LOCAL_MAP_RADIUS = remo_params['LOCAL_MAP_RADIUS']
        N_RING = remo_params['n_ring']
        N_SEG = remo_params['n_seg']
...
def get_SOI_info(node_msg, pcd_raw_points, pcd_kdtree, args):
    if args.dataset == 'kitti':
        LOCAL_MAP_RADIUS = remo_params['LOCAL_MAP_RADIUS']
        LOCAL_MAP_MIN_HEIGHT = remo_params['LOCAL_MAP_MIN_HEIGHT']
        LOCAL_MAP_MAX_HEIGHT = remo_params['LOCAL_MAP_MAX_HEIGHT']
